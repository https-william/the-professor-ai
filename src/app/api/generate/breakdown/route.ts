...
import { buildBreakdownPrompt, guardContentSize, LARGE_CONTENT_THRESHOLD } from "@/lib/ai/prompts";
...
export async function POST(req: NextRequest) {
    try {
        ...
        if (rawContent.length > LARGE_CONTENT_THRESHOLD) {
            console.info(`[Breakdown] Large content detected: ${rawContent.length} chars. Truncated: ${wasTruncated}.`);
        }

        const prompt = buildBreakdownPrompt(content, body.explainStyle);

        let aiStream: ReadableStream;
        try {
            aiStream = await hydraChatStream(
                "You are a smart study coach. Output the requested breakdown in plain markdown. Generous spacing between ## sections is mandatory.",
                [{ role: "user", content: prompt }],
                { feature: "summary", timeoutMs: 60_000 }
            );
        } catch (aiError: any) {
            console.error("Breakdown AI Error:", aiError);
        ...
        const stream = new ReadableStream({
            async start(controller) {
                try {
                    controller.enqueue(encoder.encode(`data: ${JSON.stringify({ status: "generating", message: "Deconstructing lecture notes...", wasTruncated })}\n\n`));
                    ...
                    const { data, error: dbError } = await supabase.from("generations").insert({
                        ...
                        type: "breakdown",
                        title,
                        content: { breakdown: fullMarkdown },
                    ...
                    console.error("Breakdown Stream Error:", error);
