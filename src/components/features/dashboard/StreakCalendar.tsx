...
export default function StreakCalendar({ streak, activeDates }: StreakCalendarProps) {
    ...
    return (
        <div className="p-4 rounded-2xl bg-[var(--foreground)]/[0.02] border border-[var(--border)] transition-all group hover:border-[var(--accent)]/30">
            <div className="flex items-center justify-between mb-3 px-0.5">
                <div className="flex items-center gap-1.5">
                    <Flame size={12} className="text-[var(--accent)]" />
                    <span className="font-mono text-[9px] uppercase tracking-[0.4em] text-[var(--foreground-muted)]">Momentum</span>
                </div>
                <span className="font-mono text-[10px] font-bold text-[var(--accent)] tabular-nums">{streak}d streak</span>
            </div>
                            ...
                            <div className="w-full aspect-square rounded-lg flex items-center justify-center relative transition-all">
                                {isActive && (
                                    <motion.div
                                        ...
                                        className="absolute inset-0 bg-[var(--accent)] rounded-lg shadow-[0_4px_12px_var(--accent-glow)]"
                                    />
                                )}
                                {isToday && !isActive && (
                                    <div className="absolute inset-0 border border-dashed border-[var(--accent)]/40 rounded-lg animate-pulse" />
                                )}
