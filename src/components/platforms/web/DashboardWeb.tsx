...
export default function DashboardWeb({
    ...
    // Recent generation types for visual variety
    const recentTypes = [
        { type: "flashcards", icon: Layers, color: "var(--accent)", label: "Flashcards" },
        { type: "quiz", icon: BrainCircuit, color: "#6366F1", label: "Quiz" },
        { type: "summary", icon: FileText, color: "#10B981", label: "Summary" },
    ];

    return (
        <div className="w-full min-h-screen relative bg-[var(--background)] selection:bg-[var(--accent)]/20">
            ...
            <motion.div
                ...
                className="max-w-[1200px] mx-auto px-5 md:px-10 pt-16 pb-20 relative z-10"
            >
                {/* â•â•â• HERO ZONE â•â•â• */}
                <motion.div variants={fadeUp} className="mb-8">
                    ...
                    >
                        <div className="absolute -top-20 -right-20 w-60 h-60 rounded-full blur-[100px] opacity-[0.07]"
                            style={{ background: "var(--accent)" }} />

                        <div className="relative z-10">
                            <p className="font-mono text-[10px] uppercase tracking-[0.5em] text-[var(--foreground-muted)] mb-1.5">{dateStr}</p>
                            ...
                            <p className="text-sm text-[var(--foreground-muted)] italic font-serif max-w-md">
                ...
                {/* â•â•â• STAT RIBBON â•â•â• */}
                <motion.div variants={fadeUp} className="mb-6 flex flex-wrap gap-2">
                    {[
                        { icon: Zap, label: "XP", value: user.xp?.toLocaleString(), sub: `Lvl ${level} Â· ${title}`, color: "var(--accent)" },
                        ...
                        <div key={label}
                            className="flex items-center gap-3 px-4 py-2.5 rounded-xl border transition-all group flex-1 min-w-[140px]" style={{ background: "var(--background-secondary)", borderColor: "var(--border)" }}
                        >
                ...
                {/* â•â•â• LEVEL PROGRESS (compact inline bar) â•â•â• */}
                <motion.div variants={fadeUp} className="mb-6">
                    <div className="flex items-center gap-3 px-4 py-3 rounded-xl border" style={{ background: "var(--background-secondary)", borderColor: "var(--border)" }}>
                        ...
                        <span className="font-mono text-[10px] text-[var(--foreground-muted)] tabular-nums shrink-0">
                            {xpToNext > 0 ? `${xpToNext.toLocaleString()} XP left` : "Max!"}
                        </span>
                    </div>
                </motion.div>

                {/* â•â•â• URGENT ACTIONS â•â•â• */}
                <AnimatePresence>
                    {dueCount > 0 && (
                            ...
                            <div className="flex items-center justify-between gap-4 px-4 py-3 rounded-xl bg-amber-500/8 border border-amber-500/20">
                                <div className="flex items-center gap-2.5">
                                    ...
                                    <span className="text-sm text-[var(--foreground-muted)]">
                                        Restore <b className="text-[var(--foreground)]">{user.lastStreak}d</b> streak
                                    </span>
                                </div>
                                <button onClick={handleRecover} disabled={isProcessingAction}
                                    className="px-3 py-1.5 rounded-lg bg-amber-500 text-black text-[10px] font-black uppercase tracking-wider hover:brightness-110 active:scale-95 transition-all disabled:opacity-50">
                                    Restore Â· 3 CR
                                </button>
                            </div>
                        </motion.div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                    ...
                    {/* â”€â”€ Col 1+2: Workspace â”€â”€ */}
                    <motion.div variants={fadeUp} className="lg:col-span-2 flex flex-col gap-4">

                        {/* Primary CTA */}
                        <Link href="/create" className="group block">
                            <div className="relative p-5 rounded-xl overflow-hidden transition-all hover:brightness-105" style={{ background: "color-mix(in srgb, var(--accent) 6%, var(--background-secondary))", border: "1px solid color-mix(in srgb, var(--accent) 25%, var(--border))" }}>
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                            {actions.map(({ label, desc, icon: Icon, href, accent }) => (
                                <Link key={label} href={href} className="group">
                                    <div className="p-3.5 rounded-xl h-full transition-all hover:brightness-110" style={{ background: "var(--background-secondary)", border: "1px solid var(--border)" }}>
                        ...
                        {/* Streak Calendar + Weekly Stats Row */}
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                        ...
                                        <div key={i} className="flex-1 flex flex-col items-center gap-1.5">
                                            <div className={`w-full aspect-square rounded-lg flex items-center justify-center relative transition-all ${
                                                ...
                                                <span className={`text-[9px] font-black ${
                                                    day.active ? "text-[var(--background)]" :
                            ...
                            <div className="p-4 rounded-xl" style={{ background: "var(--background-secondary)", border: "1px solid var(--border)" }}>
                                ...
                                <div className="space-y-2.5">
                                    {recentTypes.map(({ type, icon: Icon, color, label }) => (
                    ...
                    {/* â”€â”€ Col 3: Focus Panel â”€â”€ */}
                    <motion.div variants={fadeUp} className="flex flex-col gap-4">

                        {/* Pomodoro Timer */}
                        <div className="p-5 rounded-xl relative overflow-hidden" style={{ background: "var(--background-secondary)", border: "1px solid var(--border)" }}>
                            <div className={`absolute -top-12 -right-12 w-32 h-32 rounded-full blur-[60px] opacity-15 transition-colors duration-1000 ${
                                timerMode === "focus" ? "bg-[var(--accent)]" : "bg-blue-400"
                            }`} />
                            ...
                            <div className="flex flex-col items-center py-2 relative z-10">
                                ...
                                <div className="relative w-28 h-28 mb-4">
                                    <svg className="w-full h-full -rotate-90" viewBox="0 0 100 100">
                                        ...
                                        <motion.circle
                                            ...
                                            stroke={timerMode === "focus" ? "var(--accent)" : "rgb(96 165 250)"}
                                            strokeWidth="3.5"
                                            strokeLinecap="round"
                                            strokeDasharray={264}
                                            initial={{ strokeDashoffset: 264 }}
                                            animate={{ strokeDashoffset: 264 - (264 * timerProgress) }}
                                            transition={{ ease: "linear", duration: 1 }}
                                        />
                                    </svg>
                                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                                        <span className="text-xl font-black text-[var(--foreground)] tabular-nums font-mono leading-none">
                                            {timerMins.toString().padStart(2, "0")}:{timerSecs.toString().padStart(2, "0")}
                                        </span>
                                        <span className="text-[8px] text-[var(--foreground-muted)] uppercase tracking-wider mt-1">
                                            {timerMode}
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div className="flex items-center gap-2">
                                    <button
                                        ...
                                        className="w-9 h-9 rounded-xl flex items-center justify-center bg-[var(--foreground)]/5 border border-[var(--border)] active:scale-95 transition-all text-[var(--foreground)] hover:bg-[var(--accent)] hover:text-[var(--background)] hover:border-transparent"
                                    >
                                        {timerActive ? <Pause size={14} className="fill-current" /> : <Play size={14} className="fill-current ml-0.5" />}
                                    </button>
                                    <button
                                        onClick={() => { setTimerActive(false); setTimerMode("focus"); setTimerLeft(25 * 60); }}
                                        className="w-9 h-9 rounded-xl flex items-center justify-center bg-transparent border border-[var(--border)] active:scale-95 transition-all text-[var(--foreground-muted)] hover:text-[var(--foreground)]"
                                    >
                                        <RotateCcw size={12} />
                                    </button>
                                </div>
                                ...
                                <div className="mt-3 flex gap-3">
                                    <button
                                        onClick={() => { setTimerMode("focus"); setTimerLeft(25 * 60); setTimerActive(false); }}
                                        className={`font-mono text-[9px] uppercase tracking-widest transition-colors ${timerMode === "focus" ? "text-[var(--foreground)]" : "text-[var(--foreground-muted)] hover:text-[var(--foreground)]"}`}
                                    >25m Focus</button>
                                    <button
                                        onClick={() => { setTimerMode("break"); setTimerLeft(5 * 60); setTimerActive(false); }}
                                        className={`font-mono text-[9px] uppercase tracking-widest transition-colors ${timerMode === "break" ? "text-[var(--foreground)]" : "text-[var(--foreground-muted)] hover:text-[var(--foreground)]"}`}
                                    >5m Break</button>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="p-4 rounded-xl" style={{ background: "var(--background-secondary)", border: "1px solid var(--border)" }}>
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                <button
                                    ...
                                    disabled={isProcessingAction || user.credits < 5}
                                    className="px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-wider bg-cyan-500/10 border border-cyan-500/20 text-cyan-400 hover:bg-cyan-500/20 active:scale-95 transition-all disabled:opacity-30"
                                >
                                    Buy Â· 5 CR
                                </button>
                            </div>
                        ...
                        <div className="p-4 rounded-xl" style={{ background: "var(--background-secondary)", border: "1px solid var(--border)" }}>
                            ...
                            <p className="text-sm font-serif italic text-[var(--foreground)]/80 leading-relaxed">
                                &ldquo;{dailyLine}&rdquo;
                            </p>
                        </div>
                    </motion.div>
