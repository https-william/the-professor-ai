...
const QuizView = lazyRetry(() => import('./components/QuizView'));
const ProfessorView = lazyRetry(() => import('./components/ProfessorView'));
const ChatView = lazyRetry(() => import('./components/ChatView'));
const FlashcardView = lazyRetry(() => import('./components/FlashcardView'));
const AdminDashboard = lazyRetry(() => import('./components/AdminDashboard'));
const TheHub = lazyRetry(() => import('./components/TheHub'));
...
const App: React.FC = () => {
  ...
  if (currentView === 'LANDING' && !user) return <LandingPage onEnter={() => handleNavigate('AUTH', '/login')} onPricing={() => handleNavigate('PRICING', '/pricing')} onLegal={() => handleNavigate('LEGAL', '/legal')} />;
