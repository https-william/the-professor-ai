...
import { signOutUser, updateUserProfile } from './services/supabase';
...
// Lazy Components
const QuizView = React.lazy(() => import('./components/QuizView').then(m => ({ default: m.QuizView })));
const ProfessorView = React.lazy(() => import('./components/ProfessorView').then(m => ({ default: m.ProfessorView })));
const ChatView = React.lazy(() => import('./components/ChatView').then(m => ({ default: m.ChatView })));
const FlashcardView = React.lazy(() => import('./components/FlashcardView').then(m => ({ default: m.FlashcardView })));
const TheHub = React.lazy(() => import('./components/TheHub').then(m => ({ default: m.TheHub })));
const AdminDashboard = React.lazy(() => import('./components/AdminDashboard').then(m => ({ default: m.AdminDashboard })));
const SharedView = React.lazy(() => import('./components/SharedView').then(module => ({ default: module.SharedView })));
...
const App: React.FC = () => {
  ...
  const handleProcess = async (file: ProcessedFile, config: QuizConfig, mode: AppMode) => {
      try {
          ...
          const usageType = mode === 'DUEL' ? 'DUEL' : 'QUIZ';
          const updatedProfile = incrementDailyUsage(userProfile, usageType);
          ...
          if (user?.uid) {
              queueAction('UPDATE_PROFILE', { 
                  uid: user.uid, 
                  data: { 
                      dailyQuizzesGenerated: updatedProfile.dailyQuizzesGenerated,
                      dailyFilesUploaded: updatedProfile.dailyFilesUploaded,
                      lastGenerationDate: updatedProfile.lastGenerationDate
                  } 
              });
          }
    ...
    <ErrorBoundary>
        {user && onboardingStep === 'WELCOME' && (
            <WelcomeModal onComplete={async (data) => {
                ...
                saveUserProfile(updated); // Local
                
                if (user.uid) {
                    await updateUserProfile(user.uid, { ...data, hasCompletedOnboarding: true }); // Cloud
                }
        ...
        <UserProfileModal 
            ...
            profile={userProfile} 
            onSave={(u) => { setUserProfile(u); saveUserProfile(u); }} 
            onClearHistory={() => {}} 
