...
import { subscribeToDuel } from '../services/supabase';
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/95 backdrop-blur-xl animate-fade-in">
       ...
       <div className="relative bg-[#0f0f10] border border-purple-500/30 rounded-3xl p-8 max-w-lg w-full shadow-[0_0_50px_rgba(168,85,247,0.2)] text-center overflow-hidden flex flex-col max-h-[90vh]">
          ...
          <div className="mb-6 shrink-0">
             <div className="w-16 h-16 bg-purple-900/20 rounded-2xl border border-purple-500/20 flex items-center justify-center mx-auto mb-4 text-3xl shadow-[0_0_20px_rgba(168,85,247,0.3)]">⚔️</div>
          ...
          <button onClick={handleCopy} className="w-full bg-black/40 border border-purple-500/20 rounded-xl p-6 mb-6 relative group hover:bg-white/5 transition-colors text-left shrink-0">
          ...
          <button onClick={onEnter} disabled={!isReady} className="w-full py-4 bg-white text-black font-black uppercase text-xs tracking-[0.2em] rounded-xl hover:scale-[1.02] active:scale-[0.98] transition-all shadow-[0_0_30px_rgba(255,255,255,0.2)] disabled:opacity-50 disabled:cursor-not-allowed">
