...
const getWittyFeedback = (score: number, total: number) => {
    ...
    if (percentage >= 80) return ["Impressive. Most impressive.", "You might actually pass.", "Solid performance."];
    ...
    return ["I have no words. Just disappointment.", "Did you sleep through the lecture?", "A crime against academia."];
};
...
export const QuizView: React.FC<QuizViewProps> = ({ 
  ...
  const saveTextInput = () => {
      if (textAnswer.trim() && currentQ) onAnswerSelect(currentQ.id, textAnswer.trim());
  };

  const toggleMultiSelect = (opt: string) => {
      let newSelection;
      if (multiSelectAnswers.includes(opt)) newSelection = multiSelectAnswers.filter(o => o !== opt);
      else newSelection = [...multiSelectAnswers, opt].sort();
                    ...
                    <div className="max-w-2xl w-full bg-black border-2 border-red-600 rounded-3xl p-8 shadow-[0_0_100px_rgba(220,38,38,0.5)] relative overflow-hidden animate-pulse-slow">
                        ...
                        <div className="mb-8">
                            ...
                            <div className="grid grid-cols-1 gap-3">
                                {sdQ.options.map(opt => (
                                    <button key={opt} onClick={() => handleSuddenDeathAnswer(opt)} className="p-4 rounded-xl border border-red-900/50 bg-red-900/10 hover:bg-red-600 hover:text-white hover:border-red-500 transition-all text-left text-gray-300 font-medium">{opt}</button>
                                ))}
                            </div>
                        </div>
                    </div>
      ...
      <div className="max-w-5xl mx-auto pb-20 px-4 animate-fade-in">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
            <div className="glass-panel rounded-3xl p-8 flex flex-col items-center justify-center text-center relative overflow-hidden min-h-[300px]">
                ...
                <div className="bg-white/10 px-4 py-2 rounded-full text-xs font-mono text-blue-300 font-bold uppercase tracking-widest mb-6 border border-white/10">{getXPFeedback(score)}</div>
                ...
                {duelId ? (
                    <div className="flex flex-col h-full">
                        ...
                        <div className="flex-1 overflow-y-auto space-y-2">
                            {duelData?.participants.sort((a, b) => (b.score || 0) - (a.score || 0)).map((p, i) => (
                                <div key={p.id} className={`flex items-center justify-between p-3 rounded-xl border ${p.score !== undefined ? 'bg-amber-900/10 border-amber-500/20' : 'bg-white/5 border-white/5'}`}>
                                    ...
                                    <div className="text-xs font-mono font-bold text-white">{p.status === 'COMPLETED' ? `${p.score}/${total}` : '...'}</div>
                                </div>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col justify-center items-center h-full text-center">
                        ...
                        <div className="w-12 h-12 bg-amber-500/20 rounded-full flex items-center justify-center mb-6 border border-amber-500/30">
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-amber-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M12 14l9-5-9-5-9 5 9 5z" /><path d="M12 14l6.16-3.422a12.083 12.083 0 01.665 6.479A11.952 11.952 0 0012 20.055a11.952 11.952 0 00-6.824-2.998 12.078 12.078 0 01.665-6.479L12 14z" /></svg>
                        </div>
                        <p className="text-xl md:text-2xl text-amber-100 font-serif italic leading-relaxed px-4">"{feedback}"</p>
                        <div className="mt-8 text-[10px] font-bold uppercase tracking-widest text-amber-500/60">The Professor</div>
                    </div>
                )}
            </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          {questions.map((q, idx) => {
             ...
             let isCorrect = false;
             if (q.type === 'Fill in the Gap') isCorrect = userAnswer?.toLowerCase().trim() === q.correct_answer?.toLowerCase().trim();
             else if (q.type === 'Select All That Apply') isCorrect = userAnswer === JSON.stringify(safeParseJSON(q.correct_answer).sort());
             else isCorrect = userAnswer === q.correct_answer;
             ...
             return (
               <div key={q.id} className={`glass-panel rounded-2xl p-6 border-l-4 ${isCorrect ? 'border-l-green-500 bg-green-900/5' : isSkipped ? 'border-l-gray-500' : 'border-l-red-500 bg-red-900/5'}`}>
                  <div className="flex justify-between items-start mb-4">
                     ...
                     <span className={`text-[10px] font-bold uppercase px-3 py-1.5 rounded-lg border ${isCorrect ? 'bg-green-500/10 text-green-400 border-green-500/20' : isSkipped ? 'bg-gray-500/10 text-gray-400 border-gray-500/20' : 'bg-red-500/10 text-red-400 border-red-500/20'}`}>{isCorrect ? 'Correct' : isSkipped ? 'Skipped' : 'Incorrect'}</span>
                  </div>
                  <div className="text-sm text-gray-300 bg-black/30 p-4 rounded-xl border border-white/5 flex flex-col gap-3 mt-4">
                      <div className="flex gap-3">
                          <div className="flex-1">
                              ...
                              {simpleExpl ? (<div className="animate-fade-in bg-amber-900/20 p-3 rounded-lg border border-amber-500/30 text-amber-100 leading-relaxed shadow-lg"><strong className="text-amber-500 text-[9px] uppercase block mb-1">Simplification (ELI5)</strong>{simpleExpl}</div>) : (q.explanation)}
                          </div>
                      </div>
                      <div className="flex gap-2 justify-end mt-2">
                          <button onClick={() => handleELI5(q)} disabled={!!simpleExpl || loadingExplanation === q.id} className="text-[10px] font-bold uppercase bg-white/5 hover:bg-white/10 border border-white/10 px-3 py-1.5 rounded transition-colors text-gray-400 hover:text-white disabled:opacity-50">{loadingExplanation === q.id ? 'Translating...' : 'Explain Like I\'m 5'}</button>
                      </div>
                  </div>
               </div>
             );
       ...
       <div className="glass-panel p-3 sm:p-4 rounded-2xl mb-4 sticky top-4 z-30 flex flex-col gap-2 backdrop-blur-xl transition-all duration-700 border-blue-500/30 shadow-[0_0_40px_rgba(59,130,246,0.1)]">
          <div className="flex justify-between items-center px-1 sm:px-2">
             <div className="flex items-center gap-2 sm:gap-3">
                 ...
                 <span className="text-[10px] sm:text-xs font-bold uppercase tracking-widest truncate max-w-[120px] sm:max-w-none text-white">{duelId ? 'DUEL IN PROGRESS' : 'LIVE EXAM'}</span>
             </div>
             <div className="flex items-center gap-3">
                 <div className="font-mono text-sm sm:text-xl font-bold tracking-widest bg-black/40 px-3 py-1.5 rounded-lg border border-white/10 min-w-[70px] text-center text-white">{timeLeft !== null ? `${Math.floor(timeLeft / 60)}:${(timeLeft % 60).toString().padStart(2,'0')}` : '∞'}</div>
             </div>
          </div>
          <div className="flex gap-2 overflow-x-auto custom-scrollbar pb-1 pt-2 px-1 border-t border-white/5">
             {questions.map((q, idx) => {
                 ...
                 if (internalIndex === idx) statusColor = "bg-blue-600 text-white border-blue-500 ring-2 ring-blue-500/30 shadow-lg z-10";
                 else if (userAnswers[q.id]) statusColor = "bg-blue-900/20 text-blue-400 border-blue-500/30";
                 else if (isFlagged) statusColor = "bg-amber-900/20 text-amber-500 border-amber-500/30";
                 return (
                    <button key={q.id} onClick={() => handleJumpToQuestion(idx)} className={`flex-shrink-0 w-8 h-8 sm:w-9 sm:h-9 rounded-lg border flex items-center justify-center text-xs font-bold transition-all relative ${statusColor}`}>
                      {idx + 1}
          ...
          <div className="space-y-3 sm:space-y-4 flex-1 overflow-y-auto custom-scrollbar pr-2">
             {currentQ.type === 'Fill in the Gap' ? (
                 <div className="space-y-4">
                     ...
                     <input type="text" value={textAnswer} onChange={(e) => setTextAnswer(e.target.value)} onBlur={saveTextInput} className="w-full bg-transparent border-b-2 border-white/20 text-2xl py-2 px-1 text-white outline-none focus:border-blue-500 transition-colors font-mono" placeholder="Answer..." />
                 </div>
                 ...
                 <div className="grid grid-cols-1 gap-3">
                     ...
                     {currentQ.options.map((opt) => {
                         ...
                         return (
                             <button key={opt} onClick={() => toggleMultiSelect(opt)} className={`w-full text-left p-4 rounded-xl border flex items-center justify-between transition-all group ${isSelected ? 'bg-blue-600/20 border-blue-500 text-white' : 'bg-white/5 border-transparent text-gray-300 hover:bg-white/10'}`}>
                                 <span>{opt}</span>
                                 <div className={`w-5 h-5 rounded border flex items-center justify-center ${isSelected ? 'bg-blue-500 border-blue-500' : 'border-gray-500'}`}>{isSelected && <svg className="w-3 h-3 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg>}</div>
                             </button>
                         )
                     })}
                 </div>
                 ...
                 currentQ.options.map((opt) => (
                   <button key={opt} onClick={() => onAnswerSelect(currentQ.id, opt)} className={`w-full text-left p-4 sm:p-5 rounded-2xl border transition-all relative group ${userAnswers[currentQ.id] === opt ? 'bg-blue-600 text-white border-blue-500 shadow-xl shadow-blue-900/20 scale-[1.01]' : 'bg-white/5 border-transparent text-gray-300 hover:bg-white/10 hover:border-white/10'}`}>
