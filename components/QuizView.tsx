...
export const QuizView: React.FC<QuizViewProps> = ({
    ...
    const handleShareResult = async () => {
        const url = window.location.href; // Or generate a specific share link if needed for deep linking results
        const shareData = {
            title: 'Exam Result',
            text: `I just scored ${score}/${total} on The Professor AI! Can you beat me?`,
            url: url
        };
        ...
        if (navigator.share) {
            ...
            navigator.clipboard.writeText(`I just scored ${score}/${total} on The Professor AI!`);
            ...
            <div className="max-w-5xl mx-auto pb-24 px-4 animate-fade-in custom-scrollbar">
                <div className="flex justify-between items-center mb-8 sticky top-0 bg-black/80 backdrop-blur-md py-4 z-20">
                    ...
                    <div className="flex gap-2">
                        <button onClick={handleShareResult} className="px-4 py-2 bg-blue-600/20 text-blue-400 border border-blue-500/30 rounded-full font-bold uppercase text-xs hover:bg-blue-600/30">Share Result</button>
                        <button onClick={onReset} className="px-6 py-2 bg-white/5 text-white border border-white/10 rounded-full font-bold uppercase text-xs hover:bg-white/10 transition-all">Exit Session</button>
                    </div>
                </div>
        ...
        <div className="max-w-4xl mx-auto h-full flex flex-col relative">
            ...
            <div className="glass-panel p-3 sm:p-4 rounded-2xl mb-4 sticky top-4 z-30 flex justify-between items-center">
                <div className="flex items-center gap-4">
                    ...
                    <button onClick={() => setShowGrid(!showGrid)} className={`p-2 rounded-lg text-xs font-bold uppercase transition-colors ${showGrid ? 'bg-white/20 text-white' : 'bg-white/5 text-gray-400'}`}>{showGrid ? 'Hide Nav' : 'Show Nav'}</button>
                </div>
