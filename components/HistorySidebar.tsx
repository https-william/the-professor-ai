...
export const HistorySidebar: React.FC<HistorySidebarProps> = ({ isOpen, onClose, history, onSelect, onDelete }) => {
  ...
  return (
    <div className={`fixed inset-0 z-[120] overflow-hidden pointer-events-none ${isOpen ? 'pointer-events-auto' : ''}`}>
      ...
      {/* Expanded Width: sm:w-96 -> max-w-md w-full md:w-[450px] */}
      <div className={`absolute right-0 top-0 bottom-0 w-full max-w-md bg-[#18181b] border-l border-white/10 shadow-2xl transform transition-transform duration-300 flex flex-col ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <div className="p-6 border-b border-white/10 bg-[#121214]">
           ...
           <div className="relative mb-6">
               <input 
                 ...
                 className="w-full bg-black/40 border border-white/10 rounded-xl pl-10 pr-4 py-3 text-sm text-white focus:border-blue-500 outline-none placeholder-gray-600 transition-colors"
               />
               <svg xmlns="http://www.w3.org/2000/svg" className="absolute left-3 top-3.5 h-4 w-4 text-gray-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
           </div>
        ...
        {/* List */}
        <div className="flex-1 overflow-y-auto p-4 space-y-8 custom-scrollbar bg-[#0a0a0a]">
           {history.length === 0 ? (
             <div className="text-center text-gray-600 mt-20 flex flex-col items-center px-8">
               ...
               <p className="text-xs mt-1 opacity-60">Generated exams, chats, and classes will appear here automatically.</p>
             </div>
           ) : (
             Object.entries(groupedHistory).map(([label, items]) => (
               (items as HistoryItem[]).length > 0 && (
                 <div key={label}>
                   <h3 className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-4 pl-2 sticky top-0 bg-[#0a0a0a] z-10 py-2 border-b border-white/5">{label}</h3>
                   <div className="space-y-3">
                     {(items as HistoryItem[]).map((item) => (
                       <div 
                         key={item.id} 
                         className={`group p-4 rounded-xl border transition-all cursor-pointer relative hover:bg-white/5 shadow-sm hover:shadow-md ${
                       ...
                       >
                         <div className="flex items-start gap-4">
                            <div className={`mt-1 w-12 h-12 rounded-xl flex items-center justify-center shrink-0 text-xl border shadow-lg ${
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="flex justify-between items-start mb-1.5">
                                    <h4 className="text-sm font-bold text-gray-100 truncate pr-4 leading-tight">{item.title}</h4>
                                    <span className="text-[9px] text-gray-500 font-mono whitespace-nowrap shrink-0 ml-2">
                                        {getRelativeTime(item.timestamp)}
                                    </span>
                                </div>
                                
                                {item.summary && (
                                    <div className="bg-black/30 rounded px-2.5 py-1.5 mb-2 border border-white/5 inline-block max-w-full">
                                ...
                                <p className="text-[10px] text-gray-600 mt-1 truncate font-mono opacity-80 flex items-center gap-2">
                                    ...
                                    <span className="truncate">{getHistorySnippet(item)}</span>
                                </p>
                            </div>
                         </div>
                         ...
                         <button 
                            ...
                            className="absolute bottom-3 right-3 p-2 text-gray-600 hover:text-red-400 hover:bg-white/10 rounded-lg opacity-0 group-hover:opacity-100 transition-all z-20"
                            title="Delete"
                         >
        ...
        {history.length > 0 && (
            <div className="p-6 border-t border-white/10 bg-[#121214]">
                <button 
                    ...
                    className="w-full py-4 bg-red-900/10 border border-red-500/20 text-red-500 hover:bg-red-900/20 rounded-xl font-bold uppercase text-xs tracking-widest transition-all shadow-lg"
                >
                    Incinerate All Data
                </button>
            </div>
