...
import { generateSummary, callGroq } from '../services/geminiService'; // Import callGroq for review
...
export const ProfessorView: React.FC<ProfessorViewProps> = ({ state, onExit }) => {
  ...
  const [isBlurred, setIsBlurred] = useState(true); // New Tap-to-Reveal State
  ...
  const handleShare = () => {
      if (navigator.share) {
          ...
          navigator.clipboard.writeText(`Check out this lecture on ${section.title}: ${window.location.href}\n\nGenerated by The Professor AI.`);
  ...
  const handleFinish = () => {
      // Don't ask for confirmation, just exit cleanly
      onExit(true); // Assuming 'true' bypasses confirmation in parent if implemented, or we rely on parent to know context
  };
       ...
       <div className="no-print">
           <div className="flex flex-col gap-4 mb-6 border-b border-border-main pb-4">
              ...
              <div className="grid grid-cols-5 gap-2 w-full mt-2">
                 ...
                 <button onClick={handleShare} className="py-3 px-2 bg-white/5 border border-white/10 rounded-xl text-[10px] font-bold uppercase text-text-sec hover:text-white hover:bg-white/10 transition-all flex flex-col items-center justify-center gap-1.5">
