...
export const ProfessorView: React.FC<ProfessorViewProps> = ({ state, onExit }) => {
  ...
  // Logic for Math Rendering (KaTeX)
  useEffect(() => {
  ...
  }, [currentSectionIdx, state, summaryContent]);
  ...
  useEffect(() => {
    // Only cancel if manually navigating, not if podcast auto-navigates
    if (!isPodcastMode) {
        window.speechSynthesis.cancel();
        setIsSpeaking(false);
    }
  ...
  return (
    <div className="max-w-4xl mx-auto pb-24 px-4 sm:px-6 relative">
       ...
       <div id="printable-content" className="hidden">
           <h1 style={{fontSize: '24pt', marginBottom: '20px'}}>{state.sections[0]?.title || 'The Professor Lecture'}</h1>
           {state.sections.map((s, i) => (
               <div key={i} style={{marginBottom: '30px', pageBreakInside: 'avoid'}}>
                   <h2 style={{fontSize: '18pt', fontWeight: 'bold'}}>{s.title}</h2>
                   <div dangerouslySetInnerHTML={renderContent(s.content)}></div>
                   <div style={{marginTop: '10px', padding: '10px', borderLeft: '3px solid #f59e0b', background: '#fffbeb'}}>
       ...
       <div className="flex flex-col gap-4 mb-6 border-b border-white/5 pb-4 no-print">
          ...
          <div className="grid grid-cols-5 gap-2 w-full mt-2">
             <button onClick={handleSave} className={`py-3 px-2 bg-[#151515] border border-white/10 rounded-xl text-[10px] font-bold uppercase transition-all flex flex-col items-center justify-center gap-1.5 ${isSaved ? 'text-green-400 border-green-500/30' : 'text-gray-400 hover:text-white hover:bg-white/10'}`}>
                {isSaved ? (
             ...
             <button onClick={startPodcast} className={`py-3 px-2 border rounded-xl text-[10px] font-bold uppercase transition-all flex flex-col items-center justify-center gap-1.5 ${isPodcastMode ? 'bg-amber-500 text-white border-amber-500 shadow-lg shadow-amber-500/20' : 'bg-[#151515] border-white/10 text-gray-400 hover:text-white hover:bg-white/10'}`}>
             ...
             <button onClick={handleExportPDF} className="py-3 px-2 bg-[#151515] border border-white/10 rounded-xl text-[10px] font-bold uppercase text-gray-400 hover:text-white hover:bg-white/10 transition-all flex flex-col items-center justify-center gap-1.5">
       ...
       <div className="glass-panel rounded-3xl p-6 md:p-12 animate-slide-up-fade relative overflow-hidden no-print">
          {/* Section Indicator */}
       ...
       {/* Nav */}
       <div className="fixed bottom-0 left-0 right-0 p-4 bg-black/80 backdrop-blur-xl border-t border-white/10 md:static md:bg-transparent md:border-none md:p-0 md:mt-8 z-30 no-print">
