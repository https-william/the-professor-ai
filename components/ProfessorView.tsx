...
import { generateSummary, callGroq } from '../services/geminiService'; 
...
export const ProfessorView: React.FC<ProfessorViewProps> = ({ state, onExit }) => {
  ...
  const handleShare = async () => {
      ...
      const shareId = await createShareLink('PROFESSOR', state);
       ...
       <div className="no-print">
           <div className="flex flex-col gap-4 mb-6 border-b border-border-main pb-4">
              ...
              <div className="grid grid-cols-5 gap-2 w-full mt-2">
                 ...
                 <button onClick={handleShare} disabled={isSharing} className="py-3 px-2 bg-white/5 border border-white/10 rounded-xl text-[10px] font-bold uppercase text-text-sec hover:text-white hover:bg-white/10 transition-all flex flex-col items-center justify-center gap-1.5">
                    {isSharing ? (
           ...
           <div className="glass-panel rounded-3xl p-6 md:p-12 animate-slide-up-fade relative overflow-hidden">
              <div className="absolute top-4 right-6 text-[10px] font-bold font-mono text-gray-600 uppercase tracking-widest">
                 Page {currentSectionIdx + 1} of {state.sections.length}
              </div>
              ...
              <div ref={contentRef}>
                  ...
                  <div 
                    ...
                    className={`prose prose-invert prose-lg max-w-none text-text-sec leading-relaxed mb-8 text-sm md:text-base transition-all duration-700 cursor-pointer ${isBlurred ? 'blur-md select-none opacity-50 hover:opacity-70' : 'blur-0 opacity-100'}`}
                    dangerouslySetInnerHTML={renderContent(section.content)}
                  />
                  {isBlurred && (
                      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                          <span className="bg-black/80 px-4 py-2 rounded-full text-xs font-bold text-white uppercase tracking-widest animate-pulse border border-white/20">
                              Tap to Reveal
                          </span>
                      </div>
                  )}
              ...
              {section.diagram_markdown && !isBlurred && (
                 <div className="mb-8 bg-black/20 p-4 rounded-xl border border-white/5 animate-fade-in">
              ...
              <div className={`bg-amber-900/10 p-6 rounded-2xl border border-amber-500/20 mb-6 relative overflow-hidden transition-all duration-500 ${isBlurred ? 'opacity-0 translate-y-4' : 'opacity-100 translate-y-0'}`}>
           ...
           {/* FOOTER LINK */}
           <div className="text-center mt-8 mb-20 opacity-50">
               <p className="text-[10px] uppercase tracking-widest">Powered by The Professor AI</p>
           </div>
           ...
           <div className="fixed bottom-0 left-0 right-0 p-4 bg-core/80 backdrop-blur-xl border-t border-border-main md:static md:bg-transparent md:border-none md:p-0 md:mt-8 z-30">
              <div className="flex gap-4 max-w-4xl mx-auto">
                 ...
                 <button onClick={() => { if (currentSectionIdx < state.sections.length - 1) setCurrentSectionIdx(currentSectionIdx + 1); else handleFinish(); }} className="flex-[2] py-4 rounded-xl bg-amber-600 hover:bg-amber-500 text-white font-bold uppercase text-xs shadow-lg">{currentSectionIdx === state.sections.length - 1 ? 'Complete Class' : 'Next Concept →'}</button>
              </div>
           </div>
       </div>
