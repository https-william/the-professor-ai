...
export const FloatingDock: React.FC<FloatingDockProps> = ({ mode, setMode, onHub, isDuelActive, onDuel }) => {
    const DockItem = ({ active, onClick, icon, label, colorClass }: any) => (
        <button 
            onClick={onClick}
            className={`relative group flex flex-col items-center justify-center p-3 rounded-2xl transition-all duration-300 ${active ? `${colorClass} -translate-y-4 scale-110 shadow-[0_10px_30px_rgba(0,0,0,0.5)] text-white` : 'bg-white/5 hover:bg-white/10 hover:-translate-y-2 text-gray-400'}`}
        >
            <div className={`w-6 h-6 ${active ? 'text-white' : 'text-gray-400 group-hover:text-white'}`}>
                {icon}
            </div>
            <span className={`absolute -top-10 bg-black/80 text-white text-[10px] font-bold px-2 py-1 rounded-lg backdrop-blur-md whitespace-nowrap transition-opacity duration-200 pointer-events-none border border-white/10 ${active ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
                {label}
            </span>
    ...
    return (
        <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-[90] hidden md:flex items-end gap-4 px-6 pb-4 pt-4 bg-[#050505]/60 backdrop-blur-2xl border border-white/10 rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] transition-all hover:scale-105 hover:bg-[#050505]/80 ring-1 ring-white/5">
