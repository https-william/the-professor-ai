...
export const FlashcardView: React.FC<FlashcardViewProps> = ({ quizState, onExit, onGenerate, userProfile, onDeductCredits }) => {
    ...
    const handleShare = async () => {
        ...
        try {
            let url = shareUrl;
            if (!url) {
                // Title for Flashcards
                const title = `Flashcard Deck`;
                const id = await createShareLink('FLASHCARDS', title, quizState);
                if (id) {
            ...
            if (url) {
                if (navigator.share) {
                    await navigator.share({
                        ...
                        text: `Master this topic with my Flashcard Deck on The Professor AI`,
                        url: url
                    });
                } else {
