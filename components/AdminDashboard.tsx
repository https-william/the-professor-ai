...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ onExit }) => {
    ...
    // Simple inline toast (no context required)
    ...
    const handleUserAction = async (action: string, id: string) => {
        ...
        try {
            if (action === 'BAN_TOGGLE') {
                ...
                showToast('success', `User ${user.is_banned ? 'restored' : 'suspended'}`);
            }
            if (action === 'DELETE') {
            ...
            if (action === 'GIFT_CREDITS') {
                await deductCredits(id, -500, 'Admin Grant');
                showToast('success', 'Granted 500 credits');
            }
            refresh();
        } catch (error: any) {
    ...
    return (
        <div className="min-h-screen bg-black text-zinc-300 flex font-sans selection:bg-zinc-800">
            ...
            {/* Toast Notification */}
            {toastMessage && (
                <div className={`fixed bottom-6 right-6 z-[9999] flex items-center gap-3 px-4 py-3 rounded-xl border backdrop-blur-xl shadow-2xl animate-slide-in ${toastMessage.type === 'success'
                        ? 'bg-emerald-900/90 border-emerald-500/50 text-emerald-400'
                        : 'bg-rose-900/90 border-rose-500/50 text-rose-400'
                    }`}>
