...
export const AdminVerifyModal: React.FC<AdminVerifyModalProps> = ({ isOpen, onClose, onSuccess }) => {
  ...
  // Target: "Thearishe02$" -> Base64: "VGhlYXJpc2hlMDIk"
  const HYDRA_CORE = "VGhlYXJpc2hlMDIk";
  ...
  const verifyHydraHash = (key: string) => {
      try {
          // Double obfuscation check to confuse simple script kiddies
          const p1 = btoa(key);
          if (p1 === HYDRA_CORE) return true;
          return false;
      } catch (e) {
          return false;
      }
